...
def scaled_dot_product_attention(q, k, v, mask=None):
    """
    Mathematical implementation of the Transformer's core logic.
    Compatible with Python 3.14.2.
    """
    d_k = q.size(-1)
    
    # 1. Attention Scores (Raw alignment)
    ...
    # 3. Softmax (Normalization into probabilities)
    attn_weights = F.softmax(scores, dim=-1)
    
    # 4. Contextual vector (Weighted sum of values)
    output = torch.matmul(attn_weights, v)
    
    return output, attn_weights

if __name__ == "__main__":
    # Query, Key, Value vectors for a sequence of 4 tokens (dim=64)
    q = k = v = torch.randn(1, 4, 64)
    out, weights = scaled_dot_product_attention(q, k, v)
    
    print(f"Attention Output Shape: {out.shape}")
    print(f"Attention Weights Sum (Sample): {weights[0][0].sum().item():.2f}")
```
