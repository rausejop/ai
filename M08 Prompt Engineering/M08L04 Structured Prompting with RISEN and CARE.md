...
def validate_risen_structure(prompt: str):
    """
    Checks for the presence of the 5 RISEN components in a raw string.
    Compatible with Python 3.14.2.
    """
    components = {
        "Role": r"(Act as|You are a|Role:)",
        "Instruction": r"(Instruction:|Task:|Analyze|Create)",
        "Steps": r"(Step 1|Firstly|Process:)",
        "Examples": r"(Example:|Sample:)",
        "Negative Constraints": r"(Do not|Avoid|Negative Constraints:)"
    }
    
    findings = {}
    for name, pattern in components.items():
        findings[name] = bool(re.search(pattern, prompt, re.IGNORECASE))
        
    score = sum(findings.values()) / len(findings)
...
if __name__ == "__main__":
    ...
    report, final_score = validate_risen_structure(my_prompt)
    print(f"Framework Quality Score: {final_score:.0%}")
    for part, present in report.items():
        print(f" - {part}: {'[PASS]' if present else '[MISSING]'}")
```
