...
def professional_tokenizer_demo(text: str):
    """
    Demonstrates state-of-the-art tokenization with contraction handling.
    Compatible with Python 3.14 and spaCy 4.0+.
    """
    nlp = English()
    ...
    # Extract tokens with specialized metadata
    tokens = [{"text": token.text, "is_punct": token.is_punct, "is_space": token.is_space} 
              for token in doc]
    
    return tokens

if __name__ == "__main__":
    sample = "Don't forget: LLMs, as OpenAI says, are 'reasoning engines'!"
    result = professional_tokenizer_demo(sample)
    
    print(f"Original Text: {sample}")
    print(f"{'Token':<15} | {'Is Punct?':<10}")
    print("-" * 30)
    for t in result:
        print(f"{t['text']:<15} | {str(t['is_punct']):<10}")
```
