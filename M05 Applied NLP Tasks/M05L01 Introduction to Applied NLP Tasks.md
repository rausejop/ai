...
class NLPParadigm(Enum):
    DISCRIMINATIVE = "classification_labeling"
    GENERATIVE = "content_creation"

class TaskProfile(TypedDict):
    task_name: str
    paradigm: NLPParadigm
    recommended_model: str

def select_nlp_architecture(task: str) -> TaskProfile:
    """
    Automated decision engine for NLP architecture selection.
    Compatible with Python 3.14.2.
    """
    if any(q in task.lower() for q in ["classify", "detect", "filter"]):
        return {
            "task_name": task,
            "paradigm": NLPParadigm.DISCRIMINATIVE,
            "recommended_model": "RoBERTa-Base or DeBERTa-V4"
        }
    else:
        return {
            "task_name": task,
            "paradigm": NLPParadigm.GENERATIVE,
            "recommended_model": "Llama-4 or GPT-4o-mini"
        }

if __name__ == "__main__":
    tasks = ["Classify customer intent", "Summarize legal brief"]
    for t in tasks:
        profile = select_nlp_architecture(t)
        print(f"Task: {t} -> Blueprint: {profile['paradigm'].value} ({profile['recommended_model']})")
```
