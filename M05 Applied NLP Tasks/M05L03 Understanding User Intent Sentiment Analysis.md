...
def brand_reputation_analyzer(text: str):
    """
    Extracts detailed sentiment polarity for market research.
    Uses the latest RoBERTa model fine-tuned on large social datasets.
    Compatible with Python 3.14.2.
    """
    # 1. Initialize the sentiment head
    analyzer = pipeline(
        "sentiment-analysis", 
...
if __name__ == "__main__":
    sample_tweet = "The new M4 iPad is a technical marvel, but the price tag is simply absurd."
    analysis = brand_reputation_analyzer(sample_tweet)
    
    print(f"Content: {sample_tweet}")
    print(f"Aggregated Sentiment: {analysis['label']} (Prob: {analysis['score']:.4f})")
```
